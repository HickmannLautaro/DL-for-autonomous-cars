...
STATE_W = 256  # less than Atari 160x192
STATE_H = 288
VIDEO_W = 256
VIDEO_H = 288
WINDOW_W = 1000
...
class CarRacing(gym.Env, EzPickle):
    ...
    def __init__(self, verbose=1):
        ...
        self.n = 60
        self.p = 0.5
        self.tiles_to_plot = np.random.choice(a=[False, True], size= self.n * self.n, p=[self.p , 1-self.p ])
    ...
    def _create_track(self):
        ...
        border = [False] * len(track) #todo fix
        # Create tiles
        for i in range(len(track)):
    ...
    def step(self, action):
        ...
        self.state = self.render("state_pixels", draw_car=False)
        car_plot = self.render("state_pixels", draw_car=True)
        ...
        return self.state, step_reward, done, {}, car_plot, broke
    ...
    def render_segmentation(self):
        ...
        zoom = ZOOM * SCALE  # 0.1 * SCALE * max(1 - self.t, 0) + ZOOM * SCALE * min(self.t, 1)
    ...
    def render(self, mode="human", draw_car=False):
    ...
    def render_circles(self):
        if self.circle_batch != None:
            self.circle_batch.draw()
        else:

            k = PLAYFIELD / 20.0
            r = 10
            prng = RandomState(42)
            def create_circle(x, y, radius, batch, color, sides):
                circle, indices = create_indexed_vertices(x, y, radius, sides)
                vertex_count = len(circle) // 2

                #colors = [(color[0], color[1] + prng.random() / 10, color[2]) for c in vertex_count]
                colors = []#(color[0], color[1] + prng.random(), color[2])* vertex_count
                for c in range(vertex_count):
                    colors.extend([color[0], color[1] + prng.random()/3, color[2]])
                vertex_list = batch.add_indexed(vertex_count, pyglet.gl.GL_TRIANGLES, None,
                                                indices,
                                                ('v2f', circle),
                                                ('c3f', colors))

            def create_indexed_vertices(x, y, radius, sides=32):

                vertices = [x, y]
                for side in range(sides):
                    rad = radius + (radius/5) * prng.random()
                    angle = side * 2.0 * math.pi / sides
                    vertices.append(x + math.cos(angle) * rad)
                    vertices.append(y + math.sin(angle) * rad)
                # Add a degenerated vertex
                vertices.append(x + math.cos(0) * rad)
                vertices.append(y + math.sin(0) * rad)
            ...
            for x in np.linspace(-20, 20, 80):
                for y in np.linspace(-20, 20, 80):
                    shift = prng.random()*10
                    create_circle(k*x+shift, k*y+shift, prng.randint(3,6), batch, color=[0,prng.random()*0.9,0], sides=16)


            batch.draw()
            self.circle_batch = batch
    ...
    def render_seg(self):
        ...
        for poly, color in self.road_poly:
            colors.extend([1, 1, 1, 1] * len(poly))
            for p in poly:
    ...
    def render_road(self):
        ...
        n = self.n #std 20
        step = 2 #std 2 breakes if changed
        k = PLAYFIELD / n
        cols = []
        for tile in self.tiles_to_plot :
            if tile:
                cols.extend([0.4, 0.9, 0.4, 1.0]*4)
            else:
                cols.extend([0.4, 0.9, 0.4, 0.0]*4)

        #cols = [0.4, 0.9, 0.4, 1.0] * 4 * n * n

        colors.extend(cols)# + [0, 0, 0, 1.0])
        for x in range(-n, n, step):
            for y in range(-n, n, step):
                # if np.random.choice(a=[False, True], size=1, p=[p, 1-p]):
                #     pass
                polygons_.extend(
                    [
                        k * x + k,
