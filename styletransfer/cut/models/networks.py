...
def get_norm_layer(norm_type='instance'):
    ...
    elif norm_type == 'conditional_batch':
...
def init_weights(net, init_type='normal', init_gain=0.02, debug=False):
    ...
    def init_func(m):  # define the initialization function
        ...
        if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1):
            ...
            if classname != "ConditionalBatchNorm2d" and m.weight is not None:
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netG == "conditional_resnet_9":
        net = ConditionalResnetGenerator2(input_nc, output_nc, ngf, use_dropout=use_dropout,
                                          no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9,
                                          opt=opt,  # num_classes=2, conditional_layers=[2, 5, 9, 23, 27],
                                          norm_layer=norm_layer)

    elif netG == "conditional_resnet_4":
        net = ConditionalResnetGenerator2(input_nc, output_nc, ngf, use_dropout=use_dropout,
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False,
    ...
    elif netD == "conditional":
        net = NLayerConditionalDiscriminator(input_nc, ndf, n_layers=3, norm_layer=norm_layer,
...
class ConditionalGANLoss(nn.Module):
    ...
    def __call__(self, prediction, target):
        ...
        if target is None:  # no target => fake image
            target = torch.tensor([0, 0, 1]).unsqueeze(0)
        else:  # add last entry to one-hot encoding with fake=0
            fake_dim = torch.zeros([target.shape[0], 1]).to(prediction.get_device())
            target = torch.cat((target, fake_dim), dim=1)  # style1,..,stylen, fake
        target = target.unsqueeze(-1).unsqueeze(-1). \
            expand_as(prediction).to(prediction.get_device())
        target = torch.transpose(target, 1, 3).reshape(-1, 3)
        target = torch.argmax(target, dim=1)

        prediction = prediction.transpose(1, 3).reshape(-1, 3)
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
                 padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None, num_classes=1):
        ...
        Parameters:
            ...
            num_classes (int)   -- number of different possible styles
        """
        assert (n_blocks >= 0)
...
class LabelEncoder(nn.Module):
    def __init__(self, classes, width, height):
        super().__init__()
        """
        self.linear_upsample = nn.Sequential(
            nn.Linear(classes, 1024),
            nn.ReLU())
        self.conv_upsample = nn.Sequential(
            nn.Upsample(size=(64, 64)),
            nn.Conv2d(4, 16, kernel_size=(5, 5), padding_mode="zeros", padding=2),
            nn.ReLU(),

            nn.Upsample(size=(128, 128)),
            nn.Conv2d(16, 16, kernel_size=(5, 5), padding_mode="zeros", padding=2),
            nn.ReLU(),

            nn.Upsample(size=(256, 256)),
            nn.Conv2d(16, 4, kernel_size=(5, 5), padding_mode="zeros", padding=2),
            nn.Tanh()
        ...
        self.linear_embed = nn.Sequential(
            nn.Linear(classes, 256 * 256)
        )

    def forward(self, y):
        """y = self.linear_upsample(y)

        y = torch.reshape(y, (-1, 4, 16, 16))
        y = self.conv_upsample(y)"""
        # y shape: [batch_size, style1, style2, ...]

        """c = y.unsqueeze(-1).unsqueeze(-1)
        img = torch.zeros((c.shape[0], self.classes, self.height, self.width))
        c = c.expand_as(img)"""
        y = self.linear_embed(y)
        y = torch.reshape(y, shape=(-1, 1, 256, 256))
        return y


class ConditionalResnetGenerator2(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
                 padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):  # , num_classes=2):
        # conditional_layers=[]):
        """Construct a Resnet-based generator
        ...
        """
        assert (n_blocks >= 0)
        super(ConditionalResnetGenerator2, self).__init__()
        self.opt = opt
        # self.conditional_layers = conditional_layers
        # input_nc = input_nc + 1 #additonal conditional dimension
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        input_nc = input_nc + 1
        model = [LabelEncoder(classes=2, width=256, height=256),
        ...
        for i in range(n_downsampling):  # add downsampling layers
            ...
            if (no_antialias):
                model += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                          # ConditionalBatchNorm2d(ngf * mult * 2, num_classes=num_classes),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True)]
            else:
                model += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                model += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                             ...
                                             padding=1, output_padding=1,
                                             bias=use_bias),
                          # ConditionalBatchNorm2d(int(ngf * mult / 2), num_classes=num_classes),
                          norm_layer(int(ngf * mult / 2)),
                          nn.ReLU(True)]
            else:
                model += [Upsample(ngf * mult),
    ...
    def forward(self, input, layers=[], encode_only=False, encode_labels_only=False):
        x, y_oh = input

        if encode_labels_only:
            for layer_id, layer in enumerate(self.model):
                if layer_id == 0:
                    y_enc = layer(y_oh)  # torch.reshape(, (-1, 2, x.shape[2], x.shape[3]))
                    x = torch.cat((x, y_enc), dim=1)
                    # x = torch.cat((x, seg), dim=1)
                    return x

        y = torch.argmax(y_oh, dim=1)
        ...
        if len(layers) > 0:
            ...
            for layer_id, layer in enumerate(self.model):
                if layer_id == 0:
                    y_enc = layer(y_oh)  # torch.reshape(, (-1, 2, x.shape[2], x.shape[3]))
                    feat = torch.cat((feat, y_enc), dim=1)
                    #  feat = torch.cat((feat, seg), dim=1)
                    continue
                feat = layer(feat)

                """ if layer_id - 1 in self.conditional_layers:
                     feat = layer(feat, y)
                 else:"""
                if layer_id - 1 in layers:
                    ...
                    # print("%d: skipping %s %d" % (layer_id, layer.__class__.__name__, feat.size(1)))
                    pass
                if layer_id - 1 == layers[-1] and encode_only:
                    # print('encoder only return features')
                    return feats  # return intermediate features alone; stop in the last layers
            ...
            for layer_id, layer in enumerate(self.model):
                if layer_id == 0:
                    ...
                    # y_enc = torch.reshape(y_enc, (-1, 2, x.shape[2], x.shape[3]))
                    """ print(y_enc.shape)
                    print(x.shape)
...
class NLayerConditionalDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        # todo num classes!
        sequence += [
            nn.Conv2d(ndf * nf_mult, 3, kernel_size=kw, stride=1, padding=padw)]  # output 1 channel prediction map
        self.model = nn.Sequential(*sequence)
...
def define_Seg(gpu_ids=[], norm='batch', use_dropout=False, ngf=16, in_channels=3, out_channel=1):
    """
    Create class discriminator which classifies fake images into different styles.
    """
    norm_layer = get_norm_layer(norm_type=norm)
    net = UnetGenerator(in_channels, out_channel, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
